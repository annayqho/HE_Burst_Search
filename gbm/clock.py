...
def utc2gps(timestruc):
    ...
    return (timestruc - gpsref).total_seconds() + leapseconds(gpsref, timestruc)

# convert GPS seconds to UTC datetime object
def gps2utc(gps):
    leapdates_post = [timestruc for timestruc in leapdates if timestruc > gpsref]
    leapdates_gps = [(timestruc-gpsref).total_seconds()+1+i for (i, timestruc) in enumerate(leapdates_post)]
    # have two 00:00:00 times at leap second (same as xtime, not quite the same as the correct 23:59:60 just before)
    return gpsref + datetime.timedelta(seconds=gps - sum([x <= gps for x in leapdates_gps]))

# convert UTC datetime object to fermi MET (seconds since fermiref)
def utc2fermi(timestruc):
    if type(timestruc) is not datetime.datetime:
        timestruc = parsetime(timestruc)
    return (timestruc - fermiref).total_seconds() + leapseconds(fermiref, timestruc)

# convert fermi MET to UTC datetime object
def fermi2utc(met):
    leapdates_post = [timestruc for timestruc in leapdates if timestruc > fermiref]
    leapdates_met = [(timestruc-fermiref).total_seconds()+1+i for (i, timestruc) in enumerate(leapdates_post)]
    # have two 00:00:00 times at leap second (same as xtime, not quite the same as the correct 23:59:60 just before)
    return fermiref + datetime.timedelta(seconds=met - sum([x <= met for x in leapdates_met]))
...
def utc2mjd(timestruc):
    secondselapsed = (timestruc-xteref).total_seconds()
...
# Eric - I believe this is incorrect. Passing a time into utc2mjd and then that output into mjd2utc returns a different time
def mjd2utc(mjd):
